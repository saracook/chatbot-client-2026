...
workdirRoot=/z/backup-scripts/v3

# NEW: Snap support (latest snapshot by filesystem timestamp)
if [[ "$snap_enabled" == "snap" && -n "$snap_folder" ]]; then

    # Find the newest snapshot folder by actual timestamp (mtime)
    latest_snap=$(find "$snap_folder" -mindepth 1 -maxdepth 1 -type d \
    ...
    # Use the latest snapshot as source path
    path="$latest_snap"
fi
...
mkdir -p "$batchDir"
mkdir -p "$logDir"
rm "$workDir/run_this_to_copy_files.sh"
