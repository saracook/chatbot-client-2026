# Deploying a Carina User Automation API and MySQL server on Anthos Bare Metal (GKE)

This repo contains code that 
1) Deploys a MySQL server on a Anthos Bare Metal Kubernetes cluster on-premises
2) Attaches a persistent volume to it, so the data remains contained if pods are restarting
3) Deploys a User Automation API to add, delete and modify Carina users to the MySQL database and Anthos Config Management repository

## Prerequisites
1. Have `Docker CLI` (`docker`),`Kubernetes CLI` (`kubectl`), and the `Google Cloud SDK` (`gcloud`) installed on your local machine
2. Create a Google Cloud project and authenticte using the command: `gcloud init` specifying your user account and the Project ID of the Google Cloud Project
3. Authenticate to the projects Container Registry using the command: `gcloud auth configure-docker`
3. Create a Google Kubernetes Engine cluster to run the API


## Getting started
1. Clone the repository
2. Authenticate to use the private Container Registry in your Google Cloud project
3. Pull the latest mysql image from `Dockerhub`: `docker pull mysql`
4. Build a user-automation image with the Dockerfile in the api/ directory: `Docker build . -t gcr.io/PROJECT_ID/user-automation:latest`
